		...
		Run: func(conn lib.IClient, m lib.M) {
			...
			if m.IsGroup {
				if len(m.ContextInfo.MentionedJID) > 0 {
					...
					if _, err := conn.DemoteParticipant(m.Chat, jid); err != nil {
					...
					if g, err := conn.DemoteParticipant(m.Chat, jid); err != nil || g[0].Error != 0 {
						m.Reply("Terjadi Kesalahan")
						return
					}
					m.Reply("Berhasil Memberhentikan Admin")
					...
					g, err := conn.DemoteParticipant(m.Chat, jid)
					if err != nil {
						m.Reply("Terjadi Kesalahan")
						return
					}
